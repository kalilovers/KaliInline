bot = dofile('/root/inline/utils.lua')
json = dofile('/root/inline/JSON.lua')
...
BASE = '/root/inline/'
SUDO = 358945323 -- sudo id
sudo_users = {358945323,Userid}
...
bot.sendMessage(msg.chat_id_, msg.id_, 1,'<code>✅ انجام شد. ربات در گروه فعال شد </code>', 1, 'html')
end 
--------------------------rem add -------------------------
if text == 'remgp' and is_sudoers(msg) then
db:del(SUDO..'mods:'..msg.chat_id_)
db:del(SUDO..'owners:'..msg.chat_id_)
bot.sendMessage(msg.chat_id_, msg.id_, 1, '<code>>`انجام شد . لیست صاحبان و مدیران گروه خالی شد `</code>', 1, 'html')
end
--------------------------rem add ------------------------
if text == 'remall' and is_sudoers(msg) then
db:srem('bot:gps', msg.chat_id_)
db:del(SUDO..'mods:'..msg.chat_id_)
db:del(SUDO..'owners:'..msg.chat_id_)
db:del(SUDO..'banned'..msg.chat_id_)
db:del('bot:rules'..msg.chat_id_)
bot.sendMessage(msg.chat_id_, msg.id_, 1, '<code>>`DoNe ✅`</code>\n<code>`انجام شد . ربات در همه گروه ها غیرفعال گردید`</code>', 1, 'html')
end
--------------------------set link -----------------------
...
bot.sendMessage(msg.chat_id_, msg.id_, 1, '<code>>لطفا لینک گروه را ثبت کنید در غیر این صورت ربات غیرفعال باقی میماند . دستور تنظیم لینک : (لینک) setlink </code>', 1, 'html')
else
local ch = msg.chat_id_
local user_id = msg.sender_user_id_
floods = db:hget("flooding:settings:"..ch,"flood") or  'nil'
max_msg = db:hget("flooding:settings:"..ch,"تنظیم حساسیت") or 5
max_time = db:hget("flooding:settings:"..ch,"زمان حساسیت") or 5
------------------Flooding----------------------------#
...
ch = '@kalilovers'
   if result.photos_[0] then
       bot.sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,'\n👤 Your ID: '..msg.sender_user_id_..'\n🏅Your Rank: '..t..'\n➰User Msg Send :'..msgs..'\n💯Channel :'..ch)
   else
      bot.sendMessage(msg.chat_id_, msg.id_, 1, "You Have'nt Profile Photo!!\n\n👥 *👤 Your ID:* `"..msg.sender_user_id_.."`\n*🗣 Number of your Msgs: *`"..msgs.."`", 1, 'md')
   end
   end
...
-----------ban all ------------------
        ...
        local user = msg.sender_user_id_
        bot.sendMessage(msg.chat_id_, msg.id_, 1, "*اشتباه میزنی داداچ :/ رپلی کن:)*", 1, 'md')
        else
...
----------------------------------------------------------		
        if text == 'leave' then
            tdcli.changeChatMemberStatus(msg.chat_id_, 249464384, "Left")
          end
...
---------------------reload -------------------------
       ...
       dofile('./cli.lua') 
 bot.sendMessage(msg.chat_id_, msg.id_, 1,'》 <code>𝒦𝒶𝓁𝒾𝐵𝑒𝓉𝒶 TG Cli</code> <b>Reloaded  ✅</b>', 1, 'html')
            end

	   if text == 'reload inline' and is_sudo(msg) then
       dofile('./api.lua') 
 bot.sendMessage(msg.chat_id_, msg.id_, 1,'》 <code>𝒦𝒶𝓁𝒾𝐵𝑒𝓉𝒶  TG iNLiNE</code> <b>Reloaded  ✅</b>', 1, 'html')
            end
	    if text == 'kalibeta' and is_sudoers(msg) then
...
-----------del msg sudo -------------
      ...
      if  text and text:match('^del (%d+)$') then
        ...
        if limit > 1000000 then
         bot.sendMessage(msg.chat_id_, msg.id_, 1, 'بیشتر از 1000000 پیام را نمیتوانید پاک کنید', 1, 'html')
          else
------------------------------------
         function cb(a,b,c)
...
db:set(SUDO..'pinned'..msg.chat_id_,msg.reply_to_message_id_)
  ...
  if text and text == 'help' then
    if is_sudoers(msg) then
help = [[
راهنما در قسمت پنل نیز قابل مشاهده است 
...
muteall number(h|m|s) --فیلتر تمامی گفتگو ها بر حسب زمانساعت|دقیقه|ثانیه
_______________________
...
floodtime --تنظیم حساسیت نسبت به ارسال پیام مکرر برحسب زمانIn reply to 👥Translate Bot👥
___________________
>راهنمای بخش قفل ها 
